...
public class CrudCategory {
    ...
    public Category[] getCategories() {
        ...
        List<Category> activas = categories.stream()
                .filter(cat -> cat != null && cat.getStatus() != null && cat.getStatus() == 1).toList();
        if (activas.isEmpty()) {
        ...
        activas.forEach(cat -> System.out.println("{" + cat.getCategory_id() + ",\"" + cat.getCategory() + "\",\""
                + cat.getTag() + "\"," + cat.getStatus() + "}"));
    ...
    public boolean createCategory(Category category) {
        try {
            ...
            if (categories.stream()
                    .anyMatch(cat -> cat != null && (Objects.equals(cat.getCategory_id(), category.getCategory_id())
                            || Objects.equals(cat.getCategory(), category.getCategory())
                            || Objects.equals(cat.getTag(), category.getTag())))) {
                System.out.println(
                        "Categoría ya existente: no puedes crear una categoría con el mismo id, nombre de categoría o tag");
                return false;
            }
